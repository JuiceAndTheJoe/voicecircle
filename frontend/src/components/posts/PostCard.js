...
export function PostCard(post) {
  const { _id, author, content, type, mediaUrl, likesCount, commentsCount, isLiked, createdAt } = post;
...
export function attachPostCardEvents(container) {
  ...
  container.querySelectorAll('[data-like-btn]').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      const likesCountEl = container.querySelector(`[data-likes-count="${postId}"]`);
